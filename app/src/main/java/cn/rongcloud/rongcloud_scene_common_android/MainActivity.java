...
public class MainActivity extends PermissionActivity {
    ...
    protected void onAccept(boolean accept) {
        ...
        findViewById(R.id.iv_video_call).setOnClickListener(v -> {
            checkAndRequestPermissions(CALL_PERMISSIONS, new IResultBack<Boolean>() {
                @Override
                public void onResult(Boolean aBoolean) {
                    if (aBoolean) {
                        ARouter.getInstance().build(RouterPath.ROUTER_CALL).withBoolean("is_video", true).navigation();
                    } else {
        ...
        findViewById(R.id.iv_audio_call).setOnClickListener(v -> {
            checkAndRequestPermissions(CALL_PERMISSIONS, new IResultBack<Boolean>() {
                @Override
                public void onResult(Boolean aBoolean) {
                    if (aBoolean) {
                        ARouter.getInstance().build(RouterPath.ROUTER_CALL).navigation();
                    } else {
                        KToast.show("请赋予必要的权限");
                    }
                }
